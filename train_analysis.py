...
def plot_train_valid_loss(model_result_path, out_dir, model_name):
    sns.set_style(style="darkgrid")

    loss_df = pd.read_csv(f"{model_result_path}/losses.csv")
    ...
    filename_train_and_valid=f"{out_dir}/train_and_val_loss_plot_{model_name}_{neattime()}.png"
...
def plot_val_losses_per_class(model_result_path, out_dir, model_type):
    sns.set_style(style="darkgrid")
    ...
    filename_diff_immune_cell=f"{out_dir}/val_losses_per_class_plot_{neattime()}"
...
def plot_confusion_matrix(model_result_path, out_dir, model_type):
    test_predictions_df = pd.read_csv(f"{model_result_path}/test_predictions.csv")
    ...
    plt.title(f'Confusion Matrix for Leukocyte Image Classification with {model_type}')

    filename_confusion_matrix=f"{out_dir}/confusion_matrix_plot_{neattime()}.png"
...
def parse_args():
    ...
    parser.add_argument('--model_result_path', type=str, required=True, help='Path to directory containing model training results')
    parser.add_argument('--model_name', type=str, default='ResNet-18', help='Name of the model used for training')
    return parser.parse_args()


def main(args):
    out_dir = args.model_result_path + "/plots"
    os.makedirs(out_dir, exist_ok=True) 

    plot_train_valid_loss(args.model_result_path, out_dir, args.model_name)
    plot_val_losses_per_class(args.model_result_path, out_dir, args.model_name)
    plot_confusion_matrix(args.model_result_path, out_dir, args.model_name)
